...
namespace compat {
    ...
    namespace I {
        ...
        template <typename T> inline v8::Local<T> ToLocal(v8::Local<T> handle) {
            return handle;
        }

        #if !NODE_VERSION_AT_LEAST(0, 11, 0)
            template <typename T> inline v8::Local<T> ToLocal(v8::Handle<T> handle) {
    ...
    v8::Local<v8::FunctionTemplate> FunctionTemplate::New(v8::Isolate* isolate, FunctionCallback callback) {
        I::Use(isolate);
    ...
    v8::Local<v8::Integer> Integer::NewFromUnsigned(v8::Isolate* isolate, uint32_t value) {
        I::Use(isolate);
        return v8::Integer::NewFromUnsigned(COMPAT_ISOLATE_ value);
    }

    void Isolate::SetAddHistogramSampleFunction(v8::Isolate* isolate, v8::AddHistogramSampleCallback callback) {
    ...
    void Isolate::SetCreateHistogramFunction(v8::Isolate* isolate, v8::CreateHistogramCallback callback) {
        #if !NODE_VERSION_AT_LEAST(0, 11, 15)
            I::Use(isolate);
    ...
    void Isolate::SetJitCodeEventHandler(v8::Isolate* isolate, v8::JitCodeEventOptions options, v8::JitCodeEventHandler handler) {
    ...
    ReturnType ReturnableHandleScope::ThrowRangeError(v8::Local<v8::String> exception) {
    ...
    ReturnType ReturnableHandleScope::ThrowReferenceError(v8::Local<v8::String> exception) {
    ...
    ReturnType ReturnableHandleScope::ThrowSyntaxError(v8::Local<v8::String> exception) {
    ...
    ReturnType ReturnableHandleScope::ThrowTypeError(v8::Local<v8::String> exception) {
    ...
    v8::Isolate* ReturnableHandleScope::isolate() const {
        ...
        void CpuProfiler::StartCpuProfiling(v8::Isolate* isolate, v8::Local<v8::String> title) {
            if (title.IsEmpty()) title = v8::String::Empty(isolate);
            return v8::CpuProfiler::StartProfiling(title);
        }

        const v8::CpuProfile* CpuProfiler::StopCpuProfiling(v8::Isolate* isolate, v8::Local<v8::String> title) {
            if (title.IsEmpty()) title = v8::String::Empty(isolate);
            return v8::CpuProfiler::StopProfiling(title);
        }

        void Isolate::GetHeapStatistics(v8::Isolate* isolate, v8::HeapStatistics* stats) {
            I::Use(isolate);
            return v8::V8::GetHeapStatistics(stats);
        }

        v8::Local<v8::Value> Isolate::ThrowException(v8::Isolate* isolate, v8::Local<v8::Value> exception) {
            I::Use(isolate);
        ...
        v8::Local<v8::String> String::NewFromUtf8(v8::Isolate* isolate, const char* data, NewStringType type, int length) {
            I::Use(isolate);
        ...
        template <typename T> v8::Local<T> Persistent<T>::ToLocal(v8::Isolate*) const {
            return *handle_;
        }

        template <typename T> void Persistent<T>::Reset() {
            handle_.Dispose();
            handle_.Clear();
        }

        template <typename T> void Persistent<T>::Reset(v8::Isolate*, v8::Local<T> value) {
            Reset();
            handle_ = v8::Persistent<T>::New(value);
        }

        ReturnableHandleScope::ReturnableHandleScope(const ArgumentType& args) : args_(args) {
        }
        ...
        v8::Local<v8::String> String::NewFromUtf8(v8::Isolate* isolate, const char* data, NewStringType type, int length) {
            return v8::String::NewFromUtf8(isolate, data, static_cast<v8::String::NewStringType>(type), length);
        }

        HandleScope::HandleScope(v8::Isolate* isolate) : handle_scope_(isolate) {}

        template <typename T> v8::Local<T> Persistent<T>::ToLocal(v8::Isolate* isolate) const {
        ...
        template <typename T> void Persistent<T>::Reset(v8::Isolate* isolate, v8::Local<T> value) {
            handle_.Reset(isolate, value);
        }

        ReturnableHandleScope::ReturnableHandleScope(const ArgumentType& args) : args_(args), handle_scope_(args.GetIsolate()) {}
